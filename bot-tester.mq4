...
void OnTick()
  {
      ...
      //   smaF_1 = iMA(_Symbol,PERIOD_H1, 20, 20, MODE_SMA, PRICE_CLOSE, 1); // d
        ...
        // int ticket1;
         ...
         Comment("Current trend is", " " ,signal," " ,"ATR is:",SizingIndicator," ","StopLoss is:" ,StopLossModifier,"PreviousATR:",ATR_Previous,"current:",ATR_Current);
        
        // NORMAL TO HIGH Volatility 
        if(SizingIndicator >=1  && signal=="UP" && smaF_2<smaS_2 && smaF_1>smaS_1 && OrdersTotal() <=2 )
          {
            
             OrderSend(_Symbol,OP_BUY,Lot1,Ask,3,/*Ask-StopLossModifier*_Point*/ 0,Ask+150*_Point,"Buying",0,0,Green);
            
          } 
        if(SizingIndicator >=1  && signal=="DOWN" && smaF_2>smaS_2 && smaF_1<smaS_1 && OrdersTotal() <=2 )
         {
           Ticket1 = OrderSend (_Symbol, OP_SELL, Lot1, Bid, 3,/* Bid+StopLossModifier*_Point*/0, Bid-150*_Point, "On Short", 0, 0, Red);
         } 
         
         // LOW Volatility
       if(SizingIndicator == 0 && signal=="UP" && smaF_2<smaS_2 && smaF_1>smaS_1 && OrdersTotal() <=2 )
          {
            
             OrderSend(_Symbol,OP_BUY,Lot2,Ask,3,/*Ask,3,Ask-0*_Point*/ 0,Ask+100*_Point,"Buying",0,0,Green);
            
          } 
        if(SizingIndicator == 0 && signal=="DOWN" && smaF_2>smaS_2 && smaF_1<smaS_1 && OrdersTotal() <=2 )
         {
            OrderSend (_Symbol, OP_SELL, Lot2, Bid, 3, /* Bid+StopLossModifier*_Point*/0, Bid-100*_Point, "On Short", 0, 0, Red);
         } 
                    ...
                    if(OrderType() == OP_BUY)
                       {
                          if(AccountEquity()> AccountBalance())
                            {
                              bool close_buy;
                              close_buy = OrderClose(OrderTicket(),Lot1,Bid,3,CLR_NONE);
                              
                               if(close_buy == false)
                                 {
                                 Alert("Modify safety failed");
                                 }else
                                   {
                                    Alert("Modify buy success");
                                   } 
                            }
                       }
            }
                    ...
                    if(OrderType() == OP_SELL)
                       {
                            ...
                            {
                              bool close_sell;   
                              close_sell = OrderClose(OrderTicket(),Lot1,Ask,3,CLR_NONE);
                    ...
                    if(OrderType() == OP_BUY)
                       {
                              ...
                              bool close_buy_safe;
                              close_buy_safe = OrderClose(OrderTicket(),Lot1,Bid,3,CLR_NONE);
                              
                               if(close_buy_safe == false)
                                 {
                                  Alert("Modify safety failed");
                                 }else
                                   {
                                    Alert("Modify buy safety");
                                   } 
                    ...
                    if(OrderType() == OP_SELL)
                       {
                              ...
                              bool close_sell_safe;   
                              close_sell_safe = OrderClose(OrderTicket(),Lot1,Ask,3,CLR_NONE);
                              
                              if(close_sell_safe == false)
                                 {
                                  Alert("Modify safety failed");
                                 }else
                                   {
                                     Alert("Modify sell safety");
                                   }
    ...
    /*   else if(ATR_Previous>ATR_Current)
         {
          Comment("Volatility is to low !!");
          }
       else
       {
         Comment("Below Trade Limit!!");
       }     */       
  }
//+------------------------------------------------------------------+
