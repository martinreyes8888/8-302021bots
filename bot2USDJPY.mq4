        ...
        if(SizingIndicator >=1  && signal=="UP" && smaF_2<smaS_2 && smaF_1>smaS_1 && OrdersTotal() ==0 )
          {
             ...
             OrderSend("USDJPY",OP_BUY,0.03,Ask,3,0,Ask+30*_Point,"Buying",0,0,Green);
            
          } 
        if(SizingIndicator >=1  && signal=="DOWN" && smaF_2>smaS_2 && smaF_1<smaS_1 && OrdersTotal() ==0 )
         {
           Ticket1 = OrderSend ("USDJPY", OP_SELL, 0.03, Bid, 3, 0, Bid-30*_Point, "On Short", 0, 0, Red);
         } 
       ...
       if(SizingIndicator == 0 && signal=="UP" && smaF_2<smaS_2 && smaF_1>smaS_1 && OrdersTotal() ==0 )
          {
             ...
             OrderSend("USDJPY",OP_BUY,0.01,Ask,3,0,Ask+30*_Point,"Buying",0,0,Green);
            
          } 
        if(SizingIndicator == 0 && signal=="DOWN" && smaF_2>smaS_2 && smaF_1<smaS_1 && OrdersTotal() ==0 )
         {
            OrderSend ("USDJPY", OP_SELL, 0.01, Bid, 3, 0, Bid-30*_Point, "On Short", 0, 0, Red);
         } 
                    ...
                    if(OrderType() == OP_BUY)
                       {
                          if(AccountEquity()> AccountBalance())
                            {
                              bool close_buy;
                              close_buy = OrderClose(OrderTicket(),0.03,Bid,3,CLR_NONE);
                    ...
                    if(OrderType() == OP_SELL)
                       {
                            ...
                            {
                              bool close_sell;   
                              close_sell = OrderClose(OrderTicket(),0.03,Ask,3,CLR_NONE);
