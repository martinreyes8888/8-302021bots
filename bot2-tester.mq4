...
void OnTick()
  {
      ...
      //   smaF_1 = iMA(_Symbol,PERIOD_H1, 20, 20, MODE_SMA, PRICE_CLOSE, 1); // d
        ...
        if(SizingIndicator >=1  && signal=="UP" && smaF_2<smaS_2 && smaF_1>smaS_1 )
          {
            
             OrderSend("EURUSD",OP_BUY,0.03,Ask,3,0,Ask+150*_Point,"Buying",0,0,Green);
            
          } 
        if(SizingIndicator >=1  && signal=="DOWN" && smaF_2>smaS_2 && smaF_1<smaS_1 )
         {
           Ticket1 = OrderSend ("EURUSD", OP_SELL, 0.03, Bid, 3, 0, Bid-150*_Point, "On Short", 0, 0, Red);
         } 
         
         // LOW Volatility
       if(SizingIndicator == 0 && signal=="UP" && smaF_2<smaS_2 && smaF_1>smaS_1  )
          {
            
             OrderSend("EURUSD",OP_BUY,0.03,Ask,3,0,Ask+100*_Point,"Buying",0,0,Green);
            
          } 
        if(SizingIndicator == 0 && signal=="DOWN" && smaF_2>smaS_2 && smaF_1<smaS_1 )
         {
            OrderSend ("EURUSD", OP_SELL, 0.01, Bid, 3, 0, Bid-100*_Point, "On Short", 0, 0, Red);
         } 
                    ...
                    if(OrderType() == OP_BUY)
                       {
                          if(AccountEquity()> AccountBalance())
                            {
                              bool close_buy;
                              close_buy = OrderClose(OrderTicket(),0.01,Bid,3,CLR_NONE);
                    ...
                    if(OrderType() == OP_SELL)
                       {
                            ...
                            {
                              bool close_sell;   
                              close_sell = OrderClose(OrderTicket(),0.01,Ask,3,CLR_NONE);
